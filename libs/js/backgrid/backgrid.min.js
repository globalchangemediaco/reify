...
// http://es5.github.com/#x15.5.4.20
    ...
    var Cell = Backgrid.Cell = Backbone.View.extend({
        ...
        render: function () {
            this.$el.empty();
            var model = this.model;
            var text = this.formatter.fromRaw(model.get(this.column.get("name")), model) +"<div id='deleteMe'> \u24E7</div>";
            this.$el.html(text);
            this.delegateEvents();


            //ADDED by TaliSeen 07/27/16
            //if(this.column.attributes.editable){
            var self = this;
                var $del = this.$('#deleteMe');
                ...
                this.$("#deleteMe").on('click', function(e){
                    e.stopPropagation();
                    self.model.set(self.column.get('name'),null);
        ...
        enterEditMode: function () {
            ...
            if (editable) {
                ...
                var currentValueText = this.$el.text();

                model.trigger("backgrid:edit", model, column, this, this.currentEditor);

                //if ($(this.currentEditor.el).is('select')){
                //    alert('here');
                //}
                ...
                //TaliSeen BUG FIX, sometimes when you click on a select2 cell it clears the current value, this makes sure it does not
                var currentSelect2ValueID = $(_.filter(this.currentEditor.$el.find('option'),function(opt){return $(opt).html() == currentValueText})).val()
                if(currentSelect2ValueID != null){
                    //Force select2 value to be what it was before cell click, using an undocumented select2 event type the midwayers gave us
                    this.currentEditor.$el.val(currentSelect2ValueID).trigger("change.select2");
    ...
    var SelectCellEditor = Backgrid.SelectCellEditor = CellEditor.extend({
        ...
        setOptionValues: function (optionValues, editMode) {
    ...
    var SelectCell = Backgrid.SelectCell = Cell.extend({
        ...
        initialize: function (options) {
            ...
            this.listenTo(this.model, "backgrid:edit", function (model, column, cell, editor) {
                ...
                if (column.get("name") == this.column.get("name")) {
                    editor.setOptionValues(this.optionValues, self.editMode);
        ...
        render: function () {
            ...
            try {
                ...
                //ADDED by TaliSeen 07/27/16
                var text = selectedText.join(this.delimiter);
                text = text.length > 0 ? text+"<div id='deleteMe'> \u24E7</div>" : text;
