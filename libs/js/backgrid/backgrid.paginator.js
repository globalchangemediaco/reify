...
(function (root, factory) {
    ...
    var Paginator = Backgrid.Extension.Paginator = Backbone.View.extend({
        ...
        render: function () {
            ...
            $('.backgrid-paginator li' ).not('.disabled').on('click', function(e){
                var val = '';
                if( e.target.title == 'Previous'){
                    val = parseInt($('.backgrid-paginator .active').text()) - 1;
                }
                else if( e.target.title == 'First'){
                    val = 1;
                }
                else if( e.target.title == 'Next'){
                    val = parseInt($('.backgrid-paginator .active').text()) + 1;
                }
                else if( e.target.title == 'Last'){
                    val =  $('.backgrid-paginator li:nth-last-child(3)' ).text();
                }
                else{
                    val = e.target.innerHTML;
                }
                //PlantAhead.Runtime.gridView.gridFilterModel.set('filter', {'key':'page','val':val})
            });
